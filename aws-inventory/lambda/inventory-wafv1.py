# Copyright 2021 Chris Farris <chrisf@primeharbor.com>/Robert Kuhlke <bkuhlke@yahoo.com>
#
...
WAFv1_PATH = "waf/webacl"
WAFv1_REGIONAL_PATH = "waf-regional/webacl"

WAF = "AWS::WAF::WebACL"
WAF_REGIONAL = "AWS::WAFRegional::WebACL"
...
def discover_regional_WAFs(target_account, region):
    '''Find and process all the REGIONAL AWS WAFv1'''

    web_acls = []
    client = target_account.get_client('waf-regional', region=region)
    response = client.list_web_acls()
    while 'NextMarker' in response:  # Gotta Catch 'em all!
        web_acls += response['WebACLs']
        response = client.list_web_acls(NextMarker=response['NextMarker'])
    web_acls += response['WebACLs']

    logger.debug(f"Discovered {len(web_acls)} WebACLs in {target_account.account_name}")

    for acl in web_acls:
        response = client.get_web_acl(WebACLId=acl['WebACLId'])
        process_v1_acl(client, response['WebACL'], target_account, region, WAF_REGIONAL)
...
def discover_cloudfront_WAFs(target_account):
    ...
    web_acls = []
    client = target_account.get_client('waf', region='us-east-1')
    response = client.list_web_acls()
    while 'NextMarker' in response:  # Gotta Catch 'em all!
        web_acls += response['WebACLs']
        response = client.list_web_acls(NextMarker=response['NextMarker'])
    ...
    for acl in web_acls:
        response = client.get_web_acl(WebACLId=acl['WebACLId'])
        process_v1_acl(client, response['WebACL'], target_account, "global", WAF)



def process_v1_acl(client, my_WebACL, target_account, region, resourceType):
    resource_item = {}
    resource_item['awsAccountId']                   = target_account.account_id
    resource_item['awsAccountName']                 = target_account.account_name
    resource_item['resourceType']                   = resourceType
    ...
    resource_item['resourceId']                     = my_WebACL['WebACLId']
    resource_item['resourceName']                   = my_WebACL['Name']
    resource_item['ARN']                            = my_WebACL['WebACLArn']
    resource_item['errors']                         = {}

    try:
        response = client.get_logging_configuration(ResourceArn=my_WebACL['WebACLArn'])
        ...
        if e.response['Error']['Code'] == "WAFNonexistentItemException":
            ...
            message = f"Error getting the LoggingConfiguration for WebACL {my_WebACL['WebACLId']} in {region} for {target_account.account_name}: {e}"
    ...
    if region != "global":
        ...
        try:
            response = client.list_resources_for_web_acl(WebACLId=my_WebACL['WebACLId'])
            ...
            if e.response['Error']['Code'] == "WAFNonexistentItemException":
                ...
                message = f"Error getting the AssociatedResourceArns for WebACL {my_WebACL['WebACLId']} in {region} for {target_account.account_name}: {e}"
                resource_item['errors']['AssociatedResourceArns'] = message
                logger.warning(message)

    if resourceType == WAF: 
        save_resource_to_s3(WAFv1_PATH, resource_item['resourceId'], resource_item)
    else:
        save_resource_to_s3(WAFv1_REGIONAL_PATH, resource_item['resourceId'], resource_item)
